...
frappe.pages['technician_dashboard'].on_page_load = function (wrapper) {
    ...
    function show_history(history, total) {
        ...
        let total_pages = Math.ceil(total / history_page_length) || 1;
        ...
        html += `
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                <div></div>
                <div>
                    <button class="btn btn-secondary btn-sm" id="history_prev">Previous</button>
                    <span class="mx-3">Page ${history_page} of ${total_pages}</span>
                    <button class="btn btn-primary btn-sm" id="history_next">Next</button>
                </div>
            </div>
        `;
        $("#history_list").html(html);

        $("#history_prev").prop("disabled", history_page === 1).click(function () {
            if (history_page > 1) {
                history_page--;
                loadHistory();
            }
        ...
        $("#history_next").prop("disabled", history_page >= total_pages).click(function () {
    ...
    function loadProblems() {
        frappe.call({
            ...
            callback: function (r) {
                showProblems(r.message.data, r.message.total);
            }
        });
                ...
                problems.forEach(function (p) {
                    ...
                    html += `
                        <tr>
                            ...
                            <td><span class="${status_class}">${p.priority}</span></td>
                            <td>
                                <button class="btn btn-sm btn-primary view-image-btn" data-image="${p.complaint_image}">
                                    View Image
                                </button>
                            </td>
                            <td>
                                <button class="btn btn-sm btn-info update-task-btn" data-problem="${p.problem_id}">
                                    Update
                                </button>
                            </td>
                        </tr>
    ...
    function loadHistory() {
        frappe.call({
            ...
            callback: function (r) {
                show_history(r.message.data, r.message.total);
            }
        });
    }

    function open_update_dialog(selected_problem = null) {
        let task_data = [];
        ...
        frappe.call({
            ...
            callback: function (r) {
                task_data = r.message || [];
                let options = task_data.map(row => row.problem_id);
                dialog.set_df_property("problem_id", "options", options);
